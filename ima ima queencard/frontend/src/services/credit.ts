...
export async function grantCreditsInTx(
  ...
  const balance = await getCreditBalanceInTx(trx, params.userId);
...
export class CreditService {
   ...
   * 充值积分
   *
   * orderNo 只用于把积分包和业务付款流水关联起来。Stripe webhook 的幂等
   * 必须由 payment_fulfillments.fulfillmentKey 保证，不能依赖 orderNo。
   */
  async recharge(params: {
    ...
    return db.transaction(async (trx) => {
      ...
      return { packageId: grant.packageId };
    });
  }
   ...
   * 退款撤回未使用积分。
   *
   * 已消费或冻结中的积分不会被扣成负数；调用方应把未撤回部分标记为人工审核。
   */
  async refundUnusedCredits(params: {
    ...
    return db.transaction(async (trx) => {
      ...
      const requestedCredits =
        params.credits ??
        packages.reduce((sum, pkg) => sum + pkg.initialCredits, 0);
      let remainingToRefund = requestedCredits;
      ...
      await trx.insert(creditTransactions).values({
        ...
        balanceAfter: balance.availableCredits,
        orderNo: params.refundOrderNo,
        remark:
          params.remark ??
          `Refunded ${refundedCredits} unused credits for ${params.orderNo}`,
      });
   ...
   * - 配置：在 src/config/credits.ts 中统一管理
   */
  ...
  private async getBalanceInTx(
    ...
    return getCreditBalanceInTx(trx, userId);
  }
}
