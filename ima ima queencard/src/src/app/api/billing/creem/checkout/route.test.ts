...
vi.mock("@/services/billing", () => ({
  createCreemCheckout: mocks.createCreemCheckout,
}));

import { POST } from "./route";

function checkoutRequest(body: unknown, raw = false) {
  return new Request("http://localhost/api/billing/creem/checkout", {
    method: "POST",
...
describe("Creem checkout route", () => {
  ...
  it("returns a provider configuration failure as 400", async () => {
    ...
    await expect(response.json()).resolves.toEqual({
      success: false,
      error: { message: "Missing or invalid Creem product ID" },
    });
  });

  it("returns the Creem checkout URL on success", async () => {
    const response = await POST(checkoutRequest({ productKey: "creator_monthly" }));
