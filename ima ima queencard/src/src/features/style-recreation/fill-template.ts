import type { PatternValues, PatternVariable } from "./pattern-types";
...
export function renderFillTemplate(template: string, values: PatternValues) {
  return parseFillTemplate(template)
    .map((segment) =>
      segment.type === "text" ? segment.value : String(values[segment.key] ?? "")
    )
    .join("");
}
