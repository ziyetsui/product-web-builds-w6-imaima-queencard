...
export function compileStyleRecreationPrompt(input: CompileInput): CompileStyleRecreationPromptResult {
  ...
  const visualStyleAdjustments = pattern.variables
    .filter((variable) => variable.semanticRole === "visual_style" && validatedValues[variable.key] !== undefined)
    .map((variable) => `${variable.label.replace(/（可选）$/, "")}（用户指定，优先）：${formatValue(validatedValues[variable.key]!)}`);
  const visualLanguage = [
    ...visualStyleAdjustments,
