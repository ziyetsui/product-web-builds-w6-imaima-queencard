...
const pattern: StyleRecreationPattern = {
  ...
  fillTemplate: "沿用参考作品的{{visual_style}}，创作关于{{topic}}的画面，以{{emotion}}的情绪呈现{{pages}}个瞬间。",
  ...
  variables: [
    { key: "visual_style", label: "视觉风格", semanticRole: "visual_style", type: "short_text", required: true, maxLength: 50, defaultValue: "木刻、高对比和大留白" },
...
describe("StyleRecreationEditor", () => {
  it("renders one natural sentence with compact slots in reading order", () => {
    const { container } = render(<StyleRecreationEditor pattern={pattern} values={{ visual_style: "木刻、高对比和大留白", topic: "AI 创业", emotion: "焦虑", pages: 3 }} errors={{ topic: "请填写新主题" }} onValueChange={vi.fn()} />);
    const fields = Array.from(container.querySelectorAll("input, select"));
    expect(fields.map((field) => field.getAttribute("aria-label") || field.getAttribute("name"))).toEqual(["视觉风格", "新主题", "新情绪", "画面数量"]);
    expect(container).toHaveTextContent("沿用参考作品的");
    ...
    expect(screen.getByLabelText("视觉风格")).toHaveValue("木刻、高对比和大留白");
    fields.slice(1).forEach((field) => expect(field).toHaveClass("bg-lemon"));
  ...
  it("updates values without exposing the hidden compiled Prompt", () => {
    ...
    render(<StyleRecreationEditor pattern={pattern} values={{ visual_style: "木刻、高对比和大留白", topic: "AI 创业", emotion: "焦虑", pages: 3 }} errors={{}} onValueChange={onValueChange} />);
    fireEvent.change(screen.getByLabelText("视觉风格"), { target: { value: "水彩、大留白和柔和颗粒" } });
    expect(onValueChange).toHaveBeenCalledWith("visual_style", "水彩、大留白和柔和颗粒");
