...
describe("hidden style recreation prompt compiler", () => {
  ...
  it("isolates normalized user values and omits absent optional values", () => {
    ...
    expect(result.value.prompt).toContain("- 视觉风格（用户指定，优先）：粗粝版画、高对比和空间压迫感");
  ...
  it("compiles an edited visual-style field into the priority visual direction", () => {
    const result = compileStyleRecreationPrompt({
      pattern,
      values: { ...validValues, visual_style: "柔和水彩、低对比和大留白" },
    });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.prompt).toContain("- 视觉风格（用户指定，优先）：柔和水彩、低对比和大留白");
    expect(result.value.prompt).not.toContain("- 视觉风格：柔和水彩、低对比和大留白");
  });
