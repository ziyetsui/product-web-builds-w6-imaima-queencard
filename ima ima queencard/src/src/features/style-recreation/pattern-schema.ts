...
const variableBase = {
  ...
  semanticRole: z.enum(["visual_style", "content"]).optional(),
  required: z.boolean(),
...
export const styleRecreationPatternSchema = z.object({
  ...
  sourceCaseIds: z.array(nonEmpty),
  ...
  if (pattern.level === "reference" && pattern.sourceCaseIds.length !== 1) {
    context.addIssue({ code: "custom", message: "reference Patterns must bind exactly one source case", path: ["sourceCaseIds"] });
  }
